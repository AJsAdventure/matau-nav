...
def _autopilot_payload() -> dict[str, Any]:
    ...
    too. We just normalise the SignalK `state` string into our wire shape."""
    with nav_lock:
        sk_mode = nav["ap_mode"]
    ...
    mode = "wind" if (isinstance(sk_mode, str) and "wind" in sk_mode.lower()) else "compass"
    ...
    def do_POST(self):
        ...
        try:
            if path.startswith("/autopilot/"):
                cmd = path.rsplit("/", 1)[1]
                allowed = {"compass_auto", "wind_auto", "standby",
